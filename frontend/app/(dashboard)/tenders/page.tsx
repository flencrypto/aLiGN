      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-80 border-r border-border-subtle flex flex-col">
          ...
          <div className="p-4 border-b border-border-subtle">
            <div className="flex gap-2">
              ...
              <button
                ...
                className="bg-background hover:bg-surface px-3 py-2 rounded text-sm"
              >
                Go
              </button>
            </div>
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              ...
              <p className="p-4 text-text-faint text-sm">No tender awards found.</p>
            ) : (
              awards.map((award) => (
                <button
                  key={award.id}
                  onClick={() => setSelected(award)}
                  className={`w-full text-left px-4 py-3 border-b border-border-subtle/50 hover:bg-surface transition-colors ${
                    selected?.id === award.id ? 'bg-primary/20 border-r-2 border-r-blue-500' : ''
                  }`}
                >
                  <p className="text-sm font-medium text-text-main truncate">{award.winning_company}</p>
                  <p className="text-xs text-text-muted truncate">{award.authority_name}</p>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          {selected ? (
            <div className="bg-surface rounded-lg p-5">
              <div className="flex items-start justify-between mb-4">
                <div>
                  ...
                  <p className="text-text-muted text-sm">{selected.authority_name}</p>
                </div>
                <button
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  <p className="text-text-faint text-xs uppercase tracking-wide">Contract Value</p>
                  <p className="font-medium">{formatValue(selected.contract_value, selected.contract_currency)}</p>
                </div>
                <div>
                  <p className="text-text-faint text-xs uppercase tracking-wide">Award Date</p>
                  <p className="font-medium">{selected.award_date ?? '—'}</p>
                </div>
                <div>
                  <p className="text-text-faint text-xs uppercase tracking-wide">Duration</p>
                  <p className="font-medium">{selected.duration_months ? `${selected.duration_months} months` : '—'}</p>
                </div>
                <div>
                  <p className="text-text-faint text-xs uppercase tracking-wide">Region</p>
                  <p className="font-medium">{selected.region ?? '—'}</p>
                </div>
                <div>
                  <p className="text-text-faint text-xs uppercase tracking-wide">MW Capacity</p>
                  <p className="font-medium">{selected.mw_capacity != null ? `${selected.mw_capacity} MW` : '—'}</p>
                </div>
                <div>
                  <p className="text-text-faint text-xs uppercase tracking-wide">Framework</p>
                  <p className="font-medium">{selected.framework ? 'Yes' : 'No'}</p>
                </div>
              </div>
              {selected.scope_summary && (
                <div className="mt-4">
                  <p className="text-text-faint text-xs uppercase tracking-wide mb-1">Scope Summary</p>
                  <p className="text-sm text-text-main">{selected.scope_summary}</p>
                </div>
              )}
              {selected.competitors && selected.competitors.length > 0 && (
                <div className="mt-4">
                  <p className="text-text-faint text-xs uppercase tracking-wide mb-1">Competitors</p>
                  <div className="flex flex-wrap gap-2">
                    {selected.competitors.map((c) => (
                      <span key={c} className="text-xs bg-background text-text-main px-2 py-1 rounded">
                        {c}
                      </span>
                    ))}
                  </div>
            ...
            <div className="bg-surface rounded-lg p-5 text-center text-text-faint text-sm">
          ...
          <div className="bg-surface rounded-lg p-5">
            ...
            <div className="flex gap-2 mb-4">
              <input
                className="flex-1 bg-background border border-border-subtle rounded px-3 py-2 text-sm placeholder-slate-500"
              ...
              <button
                ...
                className="bg-primary hover:bg-blue-700 disabled:opacity-50 px-4 py-2 rounded text-sm font-medium"
              >
                {cpiLoading ? 'Computing…' : 'Compute CPI'}
              </button>
            </div>
            {cpiResult && (
              <div className="bg-surface/50 rounded p-4 text-sm space-y-2">
                <div className="flex justify-between">
                  <span className="text-text-muted">Company</span>
                  <span className="font-medium">{cpiResult.company}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-text-muted">Awards Analysed</span>
                ...
                {cpiResult.avg_price_per_mw != null && (
                  <div className="flex justify-between">
                    <span className="text-text-muted">Avg Price/MW</span>
          ...
          <div className="bg-surface rounded-lg p-5">
            ...
            <input
              className="w-full bg-background border border-border-subtle rounded px-3 py-2 text-sm placeholder-slate-500 mb-3"
              ...
              {relEvents.map((ev, i) => (
                <div key={i} className="flex gap-2 items-center">
                  <select
                    ...
                    className="bg-background border border-border-subtle rounded px-2 py-1.5 text-sm flex-1"
                  >
                    {SIGNAL_EVENTS.map((s) => (
                  ...
                  <input
                    ...
                    className="bg-background border border-border-subtle rounded px-2 py-1.5 text-sm w-20"
                    placeholder="Days"
                    min={0}
                  />
                  <span className="text-xs text-text-faint">days ago</span>
                  {relEvents.length > 1 && (
                    <button
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="ml-auto bg-primary hover:bg-blue-700 disabled:opacity-50 px-4 py-2 rounded text-sm font-medium"
              >
                {relLoading ? 'Analysing…' : 'Generate Brief'}
              </button>
            </div>
            {relResult && (
              <div className="mt-4 bg-surface/50 rounded p-4 text-sm space-y-3">
                ...
                <div className="flex justify-between">
                  <span className="text-text-muted">Recommend Contact</span>
                ...
                <div>
                  <p className="text-text-faint text-xs uppercase tracking-wide mb-1">Suggested Angle</p>
                  <p className="text-text-main">{relResult.suggested_angle}</p>
                </div>
                <div>
                  <p className="text-text-faint text-xs uppercase tracking-wide mb-1">Why Now</p>
                  <p className="text-text-main">{relResult.why_now}</p>
                </div>
                <div>
                  <p className="text-text-faint text-xs uppercase tracking-wide mb-1">What to Mention</p>
                  <p className="text-text-main">{relResult.what_to_mention}</p>
                </div>
                <div>
                  <p className="text-text-faint text-xs uppercase tracking-wide mb-1">What to Avoid</p>
                  <p className="text-text-main">{relResult.what_to_avoid}</p>
                </div>
                {relResult.risk_flags && (
                  <div>
                    <p className="text-text-faint text-xs uppercase tracking-wide mb-1">Risk Flags</p>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-surface rounded-xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-3">
              ...
              <div>
                <label className="text-xs text-text-muted block mb-1">Scope Summary</label>
                <textarea
                  rows={3}
                  className="w-full bg-background border border-border-subtle rounded px-3 py-2 text-sm"
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="rounded"
                />
                <label htmlFor="framework" className="text-sm text-text-main">
                  Framework Contract
                </label>
              </div>
            </div>
